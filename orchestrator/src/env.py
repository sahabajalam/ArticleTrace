...
the process working directory means a process launched from `orchestrator/`
looks for `orchestrator/.env`, finds nothing, and silently falls back to field
defaults — the same class of failure as BUG_LOG DL-025 step 2, where a service
kept talking to a dead Neo4j instance after the root `.env` had been updated.

Everything here derives from `__file__`, so the answer is the same no matter
where the process was launched from.

The knowledge engine ships the same helper (`knowledge_engine/src/env.py`). The
two services build into separate images and share no package, so the
duplication is deliberate — do not replace it with a cross-service import.
"""
