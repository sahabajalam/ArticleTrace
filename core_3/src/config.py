...
class Settings(BaseSettings):
    ...
    graphrag_api_url: str = Field(
        ...
        description="Project 3 GraphRAG API URL for legal research",
    )
    monitoring_api_url: str = Field(
        default="http://localhost:8002",
        description="Project 2 Monitoring API URL for compliance tracking",
    )
    ...
    primary_model: str = Field(default="gemini-2.0-flash", description="Primary LLM for complex reasoning")
    secondary_model: str = Field(
        default="gemini-2.0-flash", description="Secondary LLM for simple tasks"
    )
    fallback_model: str = Field(
        default="gemini-2.0-flash", description="Fallback LLM"
    )
