...
| **Single canonical external name** chosen and propagated | ✅ **Done 2026-06-16** — **AlloyCode** is canonical. Aegis deprecated. Repo + scripts + docs updated. Cloud Run URL renames correctly-deferred (Part IV). |
...
- [todo] **P0 (new, from 2026-06-19 Aura recovery)** — Push the new Aura URI/password to GCP Secret Manager and restart Cloud Run services. KE on Cloud Run still holds the phantom `652f6242` secret and will silently break when Aura hard-purges that instance. Run `./gcp.ps1 -Action secrets` then `gcloud run services update aegis-knowledge-engine aegis-orchestrator --region europe-west1 --project gdpreuai`. ~5 min.
- [todo] **P0 (from 2026-06-19 Aura recovery)** — Diagnose `keep-aura-alive.yml`: did it run, did it fail to actually reset the 30-day timer, or is Aura free-tier deletion policy-based (not activity-based)? If policy-based, retire the cron and replace with a scheduled `10_backup_to_jsonl.py` run (weekly) so the next deletion is a non-event. `gh run list -w keep-aura-alive.yml --limit 10` is the starting point.
- [todo] **P0 (from 2026-06-16 deploy verification)** — Diagnose and fix the live orchestrator's `database: unavailable` degradation (BUG_LOG DL-024). Unrelated to Aura — this is Cloud SQL / Postgres. Frontend + KE are healthy; orchestrator can't persist scans until Postgres reconnects. End-to-end demo blocked on this even though the URL is live.
- [todo] **P1 (post-expansion 2026-06-19)** — Add an entity-name index (trigram + alias) and merge as a third arm in the RRF fusion. With n=25 the gap is clear: entity recall@15 = 25% hybrid / 42% vector_only. Closes most of the structural short-label-embedding loss.
- [todo] **P1 (post-expansion 2026-06-19)** — Add a cross-regulation expansion step in `RetrievalEngine.query` that walks `COMPLEMENTS` edges one hop after the initial vector hit. Would close `GT_02` (the recurring boundary miss) and move the multi-hop category from 46% → ~55%.
...
- **[2026-06-16] Rewrite the calibration deferral rationale** — Updated `../../10Weeks/project_calibration/01_AlloyCode.md` banner. The original "sparse, empty README, no PORTFOLIO_ENTRY" reason was factually wrong; replaced with a focus-discipline rationale (Iron Rule #3) and a now-complete inventory of what AlloyCode actually has. The audit's promote-to-front-line suggestion is named explicitly as the trigger for revisiting the deferral.
...
- **Renaming `AlloyCode` again** — Pick one external name (Part II) and stop. Each rename is a free credibility hit for no architectural gain.

---

## Part V — Design rationale (why it's built this way)

Why the *shape* of AlloyCode is what it is — the principle-level reasoning that any individual code change inherits from. Not the architecture (`SYSTEM.md`); not the change history (`CHANGELOG.md`); the *why behind the why*.
...
**Defensibility over feature breadth.** Per audit §3.2: junior AI engineers are screened on execution and follow-up survival, not architecture ownership. AlloyCode is *over*-built for "junior" in scope already — the binding risk is not "too thin" but "claims that can't survive a follow-up question." Every Part III punch-list item is in service of that risk, not in service of more features.
