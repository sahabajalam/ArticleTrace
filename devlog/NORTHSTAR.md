...
|---|---|
| **Live demo URL on Cloud Run** (or a 60-second Loom of a real scan as interim) | ✅ **Fully working 2026-09-01** — all three services healthy and running current code. Was serving 2026-04-29 images until today (DL-036); the demo could never have completed a scan before now, because no database existed. Loom still pending — and only now recordable. |
...
- [done 2026-09-01] **P0 (from 2026-06-19 Aura recovery)** — Aura host `e8097dda` had been purged and no longer DNS-resolved. Rotated `NEO4J_URI`/`NEO4J_USER`/`NEO4J_PASSWORD` in Secret Manager to the live instance and redeployed. KE is `healthy`: neo4j connected, vector index online, 2,198 docs across 7 collections. Note `NEO4J_USER` is the instance id, not `neo4j`.
- [done 2026-09-01] **P0 (from 2026-06-16 deploy verification)** — `database: unavailable` was never a regression: no Cloud SQL instance ever existed and the secret held the `.env` localhost default, so the container dialled itself (DL-024). Provisioned free-tier Neon, fixed asyncpg connection-string handling (DL-037), redeployed. Orchestrator is `healthy`, and a live scan of `serengil/deepface` completed end to end on Cloud Run: 10 rules, HIGH_RISK, 87 findings, LLM triage `ok` (40 reviewed / 37 confirmed / 3 demoted), persisted to Neon.
