...
## DL-020 | "Validated 0/0 Citations" — Brittle Regex Misses Common LLM Citation Phrasings

**Date:** 2026-04-14
**Severity:** Medium (legal_research returns empty citation list despite high-quality retrieval)
**Affected:** `knowledge_engine/src/retrieval/reasoning_engine.py`
...
**Activation:** restart the knowledge_engine so the new method is loaded:
...
## DL-019 | Gemini Embedding Model `text-embedding-004` Returns 404 (Silent Empty Citations)

**Date:** 2026-04-14
**Severity:** High (legal_research agent silently produces zero citations)
**Affected:** `knowledge_engine/src/config.py`, `knowledge_engine/src/retrieval/engine.py`
...
**Why this is safe without a re-embed:** the existing JSON vector store (`knowledge_engine/chroma_data/*.json`) was already generated with `gemini-embedding-001` — embeddings are 3072-dim. Verified:
...
**Activation:** restart the knowledge_engine process so Pydantic Settings re-reads the default:
...
## DL-018 | Secrets Corrupted by PowerShell BOM / CRLF When Pushing to GCP Secret Manager

**Date:** 2026-03-25
**Severity:** Critical (breaks OAuth and any secret containing exact-match values)
**Affected:** `setup-secrets.ps1`, all Cloud Run services consuming secrets
...
**Lesson:** Never pipe secrets to `gcloud --data-file=-` on Windows PowerShell. Always write to a temp file using `WriteAllBytes` to guarantee clean byte output. This applies to any tool that reads exact-match values from Secret Manager (OAuth client IDs, API keys, JWTs, etc.).

---

## DL-017 | Gemini API Key Invalid on Cloud Run

**Date:** 2026-03-23
**Severity:** Critical (blocks all LLM operations)
**Affected:** `orchestrator` Cloud Run service
...
**Lesson:** Always test API keys independently before deploying. Secret Manager stores versions — an old version being "latest" doesn't mean the key is still valid.

---

## DL-016 | Cloud Run Domain Mapping Unsupported in europe-west2

**Date:** 2026-03-23
**Severity:** Medium
**Affected:** All Cloud Run services
...
**Note on domain mapping timing:** After adding DNS records, domain mapping verification can take 15-30 minutes. If it exceeds 1 hour, check:
...
## DL-015 | Cloud Run Rejects `PORT` as Environment Variable

**Date:** 2026-03-23
**Severity:** Medium (blocks frontend deployment)
**Affected:** `deploy.ps1` — frontend deploy command
...
## DL-014 | Cloud Run Health Checks Return 405 Method Not Allowed

**Date:** 2026-03-23
**Severity:** Low (cosmetic — services work fine)
**Affected:** `deploy.ps1` — health check section
...
## DL-013 | Cloud Run Container Fails to Start — Wrong Port

**Date:** 2026-03-23
**Severity:** High (blocks deployment)
**Affected:** All Cloud Run service deployments
...
## DL-012 | CORS Blocks Frontend → Orchestrator on Cloud Run

**Date:** 2026-03-23
**Severity:** High (blocks all API calls from deployed frontend)
**Affected:** `orchestrator/src/config.py`, `deploy.ps1`
...
## DL-011.5 | PowerShell Deploy Scripts — Multiple Pitfalls

**Date:** 2026-03-23
**Severity:** High (blocks deployment)
**Affected:** `deploy.ps1`, `deploy_gcp.ps1`, `setup-secrets.ps1`

**Symptom:** Multiple failures during GCP Cloud Run deployment:
...
**Fix:** Created `Invoke-GcloudQuery` helper that temporarily sets `$ErrorActionPreference = "Continue"` before running gcloud read-only commands.
...
**Fix:** Replaced with `Invoke-GcloudQuery { gcloud secrets describe $name }` — directly checks if the specific secret exists, no scope tricks needed.

---

## DL-011 | In-Memory Fallback Not Persisting Assessments

**Date:** 2026-03-22
**Severity:** High
**Affected:** `orchestrator/src/api/main.py`
...
## DL-010 | Windows Zombie TCP Ports (8000 and 8003)

**Date:** 2026-03-22
**Severity:** Medium
**Affected:** `start-all-modules.ps1`, `frontend/src/lib/api.ts`, `DEPLOYMENT_GUIDE.md`
...
## DL-009 | Stale Processes Block Pipeline Restart

**Date:** 2026-03-22
**Severity:** Medium
**Affected:** `start-all-modules.ps1`
...
## DL-008 | `Start-Job` Cannot Find `uv` or `npm`
...
## DL-007 | `uv run uvicorn` — "Failed to canonicalize script path"

**Date:** 2026-03-22
**Severity:** High
**Affected:** All three backend modules
...
## DL-006 | SQLAlchemy Reserved Attribute `metadata`

**Date:** 2026-03-22
**Severity:** Critical (blocks sentinel startup)
**Affected:** `sentinel/src/database/models.py`
...
## DL-005 | Orchestrator `.env` Missing `GEMINI_API_KEY`

**Date:** 2026-03-22
**Severity:** Critical (blocks orchestrator startup)
**Affected:** `orchestrator/.env`
...
## DL-004 | PowerShell Script Parse Errors
...
## DL-003 | ChromaDB Shows 0 Collections (Vector Store Misidentification)

**Date:** 2026-03-21
**Severity:** Informational (false alarm)
**Affected:** `knowledge_engine/chroma_data/`
...
**Resolution:** No fix needed — data was fully populated. The misleading directory name was kept for backward compatibility.

---

## DL-002 | `git mv` Permission Denied on Batch Rename

**Date:** 2026-03-21
**Severity:** Low
**Affected:** Repository root directories
...
## DL-001 | Frontend Hardcoded Backend URLs

**Date:** 2026-03-21
**Severity:** Medium
**Affected:** All frontend page components
...
**Additional Bug Found:** In `approvals/page.tsx`, line 14 had a string literal instead of a template literal:
