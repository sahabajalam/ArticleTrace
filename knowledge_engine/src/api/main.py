...
class GraphTraverseRequest(BaseModel):
    ...
    max_hops: int = 2
    limit: int = 20
    auto_resolve: bool = True  # Fuzzy-resolve entity names if exact ID not found
...
class EntityResolveResponse(BaseModel):
    resolved: dict[str, list[dict[str, Any]]] = Field(default_factory=dict)
    total_matched: int = 0
...
@app.post("/api/v1/graph/resolve", response_model=EntityResolveResponse)
async def resolve_entities(request: EntityResolveRequest):
    """Resolve natural language entity names to Neo4j node IDs.

    Uses pattern matching and fuzzy text search to map terms like
    "Article 9", "facial_recognition" to actual graph node IDs.
    """
    if not graph_store:
        raise HTTPException(status_code=503, detail="Graph store not available")

    resolved = graph_store.resolve_entities(
        terms=request.terms,
        limit_per_term=request.limit_per_term,
    )

    total = sum(len(matches) for matches in resolved.values())
    return EntityResolveResponse(resolved=resolved, total_matched=total)
...
async def graph_traverse(request: GraphTraverseRequest):
    ...
    When auto_resolve=True (default), natural language terms are first
    resolved to actual Neo4j node IDs before traversal.
    ...
    # Step 1: Resolve entity names to actual Neo4j IDs
    resolved_map: dict[str, list[str]] = {}
    actual_ids: list[str] = []

    if request.auto_resolve:
        resolution = graph_store.resolve_entities(request.start_entities)
        for term, matches in resolution.items():
            matched_ids = [m["id"] for m in matches]
            resolved_map[term] = matched_ids
            actual_ids.extend(matched_ids)
        # Deduplicate while preserving order
        seen_ids: set[str] = set()
        deduped_ids: list[str] = []
        for eid in actual_ids:
            if eid not in seen_ids:
                seen_ids.add(eid)
                deduped_ids.append(eid)
        actual_ids = deduped_ids
        logger.info(
            f"Resolved {len(request.start_entities)} terms -> "
            f"{len(actual_ids)} node IDs: {actual_ids[:10]}"
        )
    else:
        actual_ids = request.start_entities

    # Step 2: Traverse from resolved IDs
